...
class HiringController extends Controller
{
    ...
    public function hiring($name, $id)
    {
        ...
        return view('Template::user.hiring.request', compact('pageTitle', 'influencer', 'gatewayCurrency'));
    }
    ...
    public function hiringInfluencer(Request $request, $influencerId = 0)
    {
        ...
        $hiring = $this->saveHiringData($influencer,$user, $request, $paymentStatus);
    ...
    protected function saveHiringData($influencer,$user, $request, $paymentStatus)
    {
        $hiring                 = new Hiring();
    ...
    public function detail($id)
    {
        ...
        return view('Template::user.hiring.detail', compact('pageTitle', 'hiring'));
    }
    ...
    public function all(Request $request)
    {
        ...
        $hirings   = Hiring::searchable(['hiring_no', 'influencer:username'])->where('user_id', auth()->id())->paymentCompleted()->with('influencer', 'review')->latest()->paginate(getPaginate());
        return view('Template::user.hiring.history', compact('pageTitle', 'hirings'));
    }
    ...
    public function completeStatus($id)
    {
        ...
        $hiring->status = Status::HIRING_COMPLETED;
        $hiring->save();
    ...
    public function reportStatus(Request $request, $id)
    {
        ...
        $user           = auth()->user();
        $hiring         = Hiring::JobDone()->where('id', $id)->where('user_id', $user->id)->with('influencer')->firstOrFail();
        $hiring->status = Status::HIRING_REPORTED;
    ...
    public function conversation($id)
    {
        ...
        return view('Template::user.hiring.conversation', compact('pageTitle', 'conversationMessage', 'influencer', 'hiring'));
    }
