...
class OrderController extends Controller
{
    ...
    {
        $pageTitle = 'All Orders';
        $orders    = $this->orderData('inCompleted');
        return view('Template::user.order.list', compact('pageTitle', 'orders'));
    }
    ...
    protected function orderData($scope = null)
    {
        ...
        return $orders->searchable(['order_no', 'influencer:username'])->where('user_id', auth()->id())->with('influencer', 'review', 'service')->latest()->paginate(getPaginate());
    }
    ...
    public function order($id)
    {
        ...
        return view('Template::user.order.form', compact('pageTitle', 'service', 'influencer'));
    }
    ...
    public function orderConfirm(Request $request, $influencerId, $serviceId)
    {
        ...
        $order = $this->saveOrderData($influencer, $service, $user, $paymentStatus);

        if ($request->payment_type == 1) {
            $this->payViaWallet($user, $order, $service);
        } else {
    ...
    protected function saveOrderData($influencer, $service, $user, $paymentStatus)
    {
        $request = request();
        $order                 = new Order();
    ...
    protected function payViaWallet($user, $order, $service)
    {
    ...
    public function conversation($id)
    {
        ...
        return view('Template::user.order.conversation', compact('pageTitle', 'conversationMessage', 'influencer', 'order'));
    }
