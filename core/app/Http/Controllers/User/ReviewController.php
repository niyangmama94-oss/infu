...
class ReviewController extends Controller {
    ...
    public function index(Request $request) {
        ...
        $reviews   = Review::searchable(['review', 'hiring:hiring_no','order:order_no','influencer:username'])->where('user_id', auth()->id())->with('hiring', 'order', 'influencer')->latest()->paginate(getPaginate());
        return view('Template::user.review.index', compact('pageTitle', 'reviews'));
    }

    public function orderReviews(Request $request) {
        $pageTitle = 'My Reviews';
        $reviews   = Review::searchable(['review','order:order_no','influencer:username'])->where('user_id', auth()->id())->where('order_id', '!=', 0)->with('hiring', 'order', 'influencer')->latest()->paginate(getPaginate());
        return view('Template::user.review.index', compact('pageTitle', 'reviews'));
    }

    public function hiringReviews(Request $request) {
        $pageTitle = 'My Reviews';
        $reviews   = Review::searchable(['review','hiring:hiring_no','influencer:username'])->where('user_id', auth()->id())->where('order_id', 0)->with('hiring', 'order', 'influencer')->latest()->paginate(getPaginate());
        return view('Template::user.review.index', compact('pageTitle', 'reviews'));
    }
    ...
    public function reviewInfluencer($id) {
        ...
        return view('Template::user.review.influencer', compact('pageTitle', 'hiring'));
    }
    ...
    public function reviewService($id) {
        ...
        return view('Template::user.review.service', compact('pageTitle', 'order'));
    }
