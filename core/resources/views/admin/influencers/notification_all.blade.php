                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <select class="form-control" name="being_sent_to" required>
                                            @foreach ($notifyToInfluencer as $key => $toInfluencer)
                                                <option value="{{ $key }}">{{ __($toInfluencer) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                    ...
                                    <div class="col-md-4 start-from-col">
                                        <div class="form-group">
                                            ...
                                            <input class="form-control" name="start_form" type="number"
                                                placeholder="@lang('Start form influencer')" required />
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                <input class="form-control" name="batch" type="number"
                                                    placeholder="@lang('How many influencer')" required />
                ...
                <div class="modal-body">
                    ...
                    <div class="mt-3">
                        <p class="sentStatistics text-center mb-2">@lang('Email sent') <span class="startFrom">0</span>
                            @lang('to') <span class="sent">-</span> @lang('Influencers')
                        </p>
    ...
    <script>
        (function($) {
            ...
            var subject = null,
                ...
                _token = null,
                influencer = null,
                number_of_top_deposited_influencer,
                number_of_days
            $('.notify-form').on('submit', function(e) {
                ...
                influencer = $(".input-append").find(`#influencer_list`).val();


                number_of_top_deposited_influencer = $(".input-append").find(
                    'input[name=number_of_top_deposited_influencer]').val();
                number_of_days = $(".input-append").find('input[name=number_of_days]').val();
                _token = $(this).find('[name=_token]').val();
                if ({{ $influencers }} <= 0) {
                    notify('error', 'Influencers not found');
                    return false;
                }
                if (!coolingTime) {
            ...
            function postMail() {
                ...
                $.post("{{ route('admin.influencers.notification.all.send') }}", {
                    "subject": subject,
                    "_token": _token,
                    "start": start,
                    "batch": perBatch,
                    ...
                    'number_of_top_deposited_influencer': number_of_top_deposited_influencer,
            ...
            $('select[name=being_sent_to]').on('change', function(e) {
                ...
                if (methodName == 'SELECTEDINFLUENCERS') {
                    $('.input-append').html(`
                    <div class="form-group" id="influencer_list_wrapper">
                        <label class="required">@lang('Select User')</label>
                        <select name="influencer[]"  class="form-control" id="influencer_list" required multiple>
                ...
                if (methodName == 'TOPDEPOSITEDINFLUENCERS') {
                    $('.input-append').html(`
                    <div class="form-group">
                        <label class="required">@lang('Number Of Top Deposited User')</label>
                        <input class="form-control" type="number" name="number_of_top_deposited_influencer" >
                    </div>
                    `);
                    changeEmailSendingOptionHtml(true);
                    return
                ...
                if (methodName == 'NOTLOGININFLUENCERS') {
                    $('.input-append').html(`
                    <div class="form-group">
            ...
            function fetchInfluencerList() {

                $('.row #influencer_list').select2({
                    ajax: {
                        url: "{{ route('admin.influencers.list') }}",
                        type: "get",
                        dataType: 'json',
                        delay: 1000,
                        ...
                        processResults: function(response, params) {
                            ...
                            let data = response.influencers.data;
                    ...
                    dropdownParent: $('.input-append #influencer_list_wrapper')
                });
            }
